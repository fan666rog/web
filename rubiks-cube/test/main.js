...
// --- Gizmo 設定 ---
let gizmoRenderer, labelRenderer, gizmoScene, gizmoCamera, gizmoAxes, gizmoRaycaster;
...
function initGizmo() {
    ...
    axes.forEach(axisInfo => {
        ...
        arrow.name = axisInfo.name; // For raycasting
        gizmoAxes.add(arrow);
    ...
    gizmoRaycaster.params.Line.threshold = 0.2;
    labelRenderer.domElement.addEventListener('pointerdown', onGizmoClick);
}

function onGizmoClick(event) {
    if (isAnimating) return;

    const gizmoBounds = labelRenderer.domElement.getBoundingClientRect();
    ...
    const intersects = gizmoRaycaster.intersectObjects(gizmoAxes.children.filter(c => c.type === 'ArrowHelper'));

    if (intersects.length > 0) {
        const axisName = intersects[0].object.name;
...
function animate() {
    ...
    if (gizmoRenderer) {
        gizmoAxes.quaternion.copy(camera.quaternion);
